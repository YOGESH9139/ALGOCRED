...
export default function CreateBountyPage() {
  ...
  const handleSubmit = async () => {
    if (validateStep(currentStep)) {
      ...
      // Redirect to mock bounty
      router.push('/bounties/mock-new-bounty')
    }
  }
            ...
            {steps.map((step, index) => (
              <React.Fragment key={step}>
                ...
                {index < steps.length - 1 && (
                  <div className={`w-8 h-0.5 ${index < currentStep ? 'bg-electric-cyan' : 'bg-charcoal-steel'}`} />
                )}
              </React.Fragment>
            ))}
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="glass p-3 border border-electric-cyan/30 rounded">
                <p className="text-muted-silver text-sm">
                  Uses Algorand stablecoins (mock). Funds will be held in escrow until work is approved.
                </p>
              </div>
            </div>
          )}
